...
create table IF NOT EXISTS resuser(
...
create table  IF NOT EXISTS resfood(
...
create table IF NOT EXISTS resorder(
...
--    add constraint if not exists fk_resorderitem_roid
--       foreign key(roid) references resorder( roid);
--
--  alter table resorderitem
--    add constraint if not exists fk_tbl_res_fid
--       foreign key( fid ) references resfood( fid);
     

      
create table IF NOT EXISTS resorderitemtemp(
